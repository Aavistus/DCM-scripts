...
%     DCM.A{forward}(rSP,Prec) = 1; %CEN-DMN
    DCM.A{forward}(Prec,lSP) = 1; %DMN-CEN 
    DCM.A{forward}(Prec,rSP) = 1; %DMN-CEN 
    DCM.A{forward}(dACC,Prec) = 1; %DMN-SAL
    
%     DCM.A{backward}(Prec,lSP) = 1; %DMN-CEN
%     DCM.A{backward}(Prec,rSP) = 1; %DMN-CEN
    DCM.A{backward}(lSP,Prec) = 1; %CEN-DMN 
    DCM.A{backward}(rSP,Prec) = 1; %CEN-DMN 
    DCM.A{backward}(Prec,dACC) = 1; %SAL-DMN
...
elseif modindex == 5 
    ...
    DCM.A{forward} = zeros(Nareas,Nareas);
    DCM.A{forward}(Prec,lSP) = 1; %CEN-DMN
    DCM.A{forward}(Prec,rSP) = 1; %CEN-DMN
    DCM.A{forward}(dACC,Prec) = 1; %DMN-SAL
    
    DCM.A{backward} = zeros(Nareas,Nareas);
    DCM.A{backward}(lSP,Prec) = 1; %DMN-CEN
    DCM.A{backward}(rSP,Prec) = 1; %DMN-CEN
    DCM.A{backward}(Prec,dACC) = 1; %SAL-DMN
...
%DCM.options.model   = 'CMC';  % CMC model - better for intrinsic conn.
...
% DCM.options.trials  = [1];   % This when fitting on conditions separately
DCM.options.trials   = [1 2];  % Select specific trials
...
DCM.options.Tdcm(2)  = 10000;  % end of peri-stimulus time to be modelled (ms)
                               % Note: here epoch length (10 seconds)  
...
%contrast vector
% DCM.xU.X = [];    % This when fitting conditions separately
DCM.xU.X = [0 1]';  % This when fitting into a contrast
...
save([filepath14 datafile '_model' (models{modindex}) '.mat'],'DCM');
